...
#import "PKReadDetailReadModel.h"
...
@interface PKReadListDetailController ()<UIScrollViewDelegate,PKReadDetailReadTableViewDelegate>

@property (nonatomic,strong) PKReadDetailReadTableView *tableView;
@property (nonatomic,strong) NSMutableArray *tableSource;
@property (nonatomic,strong) NSMutableArray *cellHeights;
@property (nonatomic,strong) PKReadDetailReadTableView *hotTableView;
...
@implementation PKReadListDetailController
...
- (void)viewDidLoad {
    ...
    _tableView = [[PKReadDetailReadTableView alloc] initWithFrame:CGRectMake(0, 0, VIEW_WIDTH, VIEW_HEIGHT - 65) style:UITableViewStylePlain];
    _tableView.articleDelegate = self;
    [self.backScrollView addSubview:_tableView];
    
    _hotTableView = [[PKReadDetailReadTableView alloc] initWithFrame:CGRectMake(VIEW_WIDTH, 0, VIEW_WIDTH, VIEW_HEIGHT - 65) style:UITableViewStylePlain];
...
- (void)postForInfo{
    ...
    [self POSTHttpRequest:@"http://api2.pianke.me/read/columns_detail" dic:dic successBalck:^(id JSON) {
        
        if ([JSON[@"result"] integerValue]) {
            PKReadDetailReadModel *model = [[PKReadDetailReadModel alloc] initWithDictionary:JSON];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [_naviBar.nameLabel setText:model.data.columnsInfo.typename];
            });
        }
...
- (void)hotLoadMoreTableViewData {
    NSDictionary *dic = @{
                          ...
                          @"limit" : @"10",
                          @"sort" : @"hot",
                          @"start" : [NSString stringWithFormat:@"%ld",self.hotCellHeights.count],
                          @"typeid" : self.typeId,
                          @"version" : @"3.0.6",
                          };
...
#pragma mark - PKReadDetailReadTableViewDelegate
